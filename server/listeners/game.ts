...
export const acknowledgeShotListener: SocketListener = {
    ...
    callable: (_, channel: string) => {
        ...
        if (pendingShot.chat) require('../modules/twitchBot').say(pendingShot.chat, channel);
        ...
        gameManager.getAggregatedData(channel)
            .then(({ settings, roundInfo }: AggregatedResponse) => {
                ...
                if (!shot) twitchAPI.say(HELP_MSG, user);
                    ...
                    if (settings.chat.responseEnabled && roundInfo.inProgress && !gameManager.delayedGames.includes(channel)) twitchAPI.say(settings.chat.inProgressMessage.replace('@user', `@${user}`), channel);
                    else {
                        //EMIT SOCKET INFO
