...
import { ShotResult, AggregatedResponse, PendingShot, GameManager, SettingsInfoResponse, LoggedShotResponse } from '../types/game';
...
const gameManager: GameManager = module.exports = {
    ...
    delayedGames: [],
    addListeners() {
        const { joinListener, leaveListener, acknowledgeShotListener, chatShotListener } = require('../listeners/game');
        ...
        return new Promise((resolve) => {
            ...
            settingsCollection.aggregate([
                ...
                if (data.length != 0) {
                    ...
                    mongoAPI.db && mongoAPI.db.collection(process.env.SETTINGS_COLLECTION_NAME || 'settings').insertOne(settings); //why is ts making me check for db again?
                    newUser = true;
                }
                if (data[0] && data[0].roundInfo.length != 0) {
                    ...
                    mongoAPI.db && mongoAPI.db.collection(process.env.ROUND_COLLECTION_NAME || 'roundinfo').insertOne(roundInfo);
                };

                resolve({ settings: settings, roundInfo: roundInfo, newData: newUser });
            })
        });
    },
